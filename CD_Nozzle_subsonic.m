...
for n=1:n_tot-1
    ...
    %predictor step (note functions 'fd' and 'bd' defined at the end)
        ...
        rho(n+1,i_tot)=p_e/T(n+1,i_tot);
        V(n+1,i_tot)=2*V(n+1,i_tot-1)-V(n+1,i_tot-2);
end

plot(x,0.5*A,x,-0.5*A)
ylabel('A/A_t','FontSize',15),xlabel('x/L')
...
%------------Analytic solution---------------------------------------
%find mach number M_e at exit based on p_e/p0 using isentropic relations
M_e=sqrt((p_e^((-gamma+1)/gamma)-1)*2*inv(gamma-1));
%use M_e to find A* (sonic area) which is constant throughout the flow
A_s=A(end)^2*M_e^2*( (2/(gamma+1))*(1+0.5*M_e^2*(gamma-1)))^((-gamma-1)/(gamma-1));  %area M relation
A_s=sqrt(A_s);
%scale area with A*
A2=A/A_s;

%find analytic mach number using area mach number area relation for A2
    for i=1:i_tot
